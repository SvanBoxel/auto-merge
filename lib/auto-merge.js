...
module.exports = async (context) => {
  ...
  const strategy = await withConfig(context)(getStrategy)
  if (strategy === 'label' && !labels.find(({ name }) => name === 'auto-merge')) {
  ...
  const merge_method = await withConfig(context)(getMergeMethod)
